...
public class Ejercicio12 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("\n La tabla con los números que ha introducido "
            + "queda así ");
      ...
      for (int i = 0; i <= (numeros.length - 1); i++) {
        ...
        switch (j) {
          ...
          case 2:
            ...
            System.out.print(i);
            for (int k = 1; k <= (longitud - mitad); k++) {
              System.out.print(" ");
            }
            System.out.print(" ");
            if (i == (numeros.length - 1)) {
    ...
    do {
      System.out.print("¿A que posición (0 - 9) mayor que la anterior quiere"
              + " mover el valor? ");
      posFinal = s.nextInt();
      ...
      if (posFinal < posInicial) {
        System.out.println("\nLa posición a la que mover el valor debe ser"
                + " mayor a la posición"
                + " actual");
      }
    ...
    for (int i = 0; i <= (temp.length - 1); i++) {
      if (i == posFinal) {
        temp[i] = valor;
      } else if ((i > posInicial) && (i < posFinal)) {
        temp[i] = numeros[i];
      } else {
        if (i == 0) {
          temp[i] = numeros[numeros.length - 1];
        } else {
          temp[i] = numeros[i - 1];
        }
      }
    ...
    System.out.println("\nTras mover el valor de la posición " + posInicial
            + " a la posición "
